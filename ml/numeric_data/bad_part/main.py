...
from visualization import plot_the_dataset, plot_a_contiguous_portion_of_dataset
...
def main():
    ...
    # 使用 io.StringIO 將 dataset 字串轉換為類似檔案的對象，並讀取成 pandas DataFrame
    # 這樣就可以直接從字串資料中讀取 CSV 格式的數據

    training_df = pd.read_csv(io.StringIO(dataset), on_bad_lines='warn')
    # 'on_bad_lines="warn"' 參數告訴 pandas 在遇到錯誤行（例如格式錯誤或列數不一致）時顯示警告，
    # 但不會停止執行，讓程式繼續處理其他正確的資料行。

    print(training_df.describe())

    print("""
    基本統計數據顯示沒有太多離群值
    標準差明顯小於平均值，表示數據分布較為集中
    此外，四分位數的界限大致均勻分布，顯示數據沒有極端偏差""")

    plot_the_dataset(training_df, "calories", "test_score", number_of_points_to_plot=50)

    print("""
    視覺化 50 個數據點時，並不會顯示出任何離群值。 
    然而，當你增加隨機繪製的數據點數量後，會出現一群離群值。 
    請注意那些測試分數很高但熱量卻低於 200 的數據點""")
    ...
    print("=== 700 到 1050 筆資料 ===")
    print(training_df[700:1050].describe())

    print("=== 1050 到 1400 筆資料 ===")
    print(training_df[1050:1400].describe())

    print("""每週的基本統計數據相似，因此離群值不太可能是由每週的差異造成的""")

    for i in range(0, 7):
        start = i * 50
        end = start + 49
        plot_a_contiguous_portion_of_dataset(training_df, "calories", "test_score", start, end, i)

    print("""從第一週可以看到，第 4 天的熱量範圍只有 0 到 200，其他天的熱量範圍卻是 0 到 400。看起來第 4 天有點問題""")
    ...
    print("星期四的平均熱量是 %.0f" % (mean_of_thursday_calories))
    print("非星期四日的平均熱量是 %.0f" % (mean_of_non_thursday_calories))
